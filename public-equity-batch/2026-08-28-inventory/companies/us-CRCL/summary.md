...
- 健檢動作：續抱
- 健檢理由：研究評級為續抱且價格 normal，可保留既有部位；利率與分潤敏感仍不支持加碼
- 近一週觸發：近一週未觸發買進/賣出
- 短線情境：high 96.24；warn 91.43；break 86.62；目前 normal
- 失效條件：USDC circulation／RLDC margin、非利息收入與 Arc 兌現改善可上調；跌破 86.62 時重評
...
- 風險預算：0.0
- 下檔/停損距離：20d_low distance 38.62%; 20D low 57.84 (input only; PM must verify)
- 流動性上限：20D avg trade value 1,117,042,971.65; PM must define liquidity cap
...
- 減碼規則：研究評級為續抱且價格 normal，可保留既有部位；利率與分潤敏感仍不支持加碼
- 退出規則：USDC circulation／RLDC margin、非利息收入與 Arc 兌現改善可上調；跌破 86.62 時重評
