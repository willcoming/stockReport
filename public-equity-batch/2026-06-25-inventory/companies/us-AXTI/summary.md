...
- 目前價格：70.14 USD
- 價格時間：2026-06-24
- 來源：Yahoo Finance via yfinance AXTI (AXTI)
...
- 健檢動作：觀察
- 健檢理由：PEI 證據與 APEX 門檻不足，先觀察。
- 近一週觸發：近一週觸發資料未提供
- 短線情境：APEX 資料不足：多/空/震盪 25/25/50；缺數值停損門檻
- 失效條件：PEI：來源未提供，需後續查證。；APEX：缺數值支撐/破位門檻，需補獨立圖表支撐。
...
- 減碼規則：PEI 證據與 APEX 門檻不足，先觀察。
- 退出規則：PEI：來源未提供，需後續查證。；APEX：缺數值支撐/破位門檻，需補獨立圖表支撐。
