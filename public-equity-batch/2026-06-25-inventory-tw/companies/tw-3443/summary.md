...
- 健檢動作：觀察
- 健檢理由：缺 APEX 數值 stop-loss，先觀察。
- 近一週觸發：近一週未觸發買進/賣出
- 短線情境：APEX 資料不足：多/空/震盪 25/25/50；缺數值停損門檻
- 失效條件：PEI：高估值無法由營收/訂單上修支撐。；APEX：缺數值支撐/破位門檻，需補獨立圖表支撐。
...
- 減碼規則：缺 APEX 數值 stop-loss，先觀察。
- 退出規則：PEI：高估值無法由營收/訂單上修支撐。；APEX：缺數值支撐/破位門檻，需補獨立圖表支撐。
