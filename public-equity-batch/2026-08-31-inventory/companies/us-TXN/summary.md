...
- 健檢動作：續抱
- 健檢理由：基本面支持保留，但技術明確偏空，禁止加碼
- 近一週觸發：近一週未觸發買進/賣出
- 短線情境：high 289.62；warn 275.14；break 260.66；目前 break
- 失效條件：工業車用需求、庫存、FCF、12吋廠利用率或併購惡化時下調；站回 275.14 才解除警戒
...
- 風險預算：0.0
- 下檔/停損距離：20d_low distance 1.34%; 20D low 255.18 (input only; PM must verify)
- 流動性上限：20D avg trade value 1,597,139,850.15; PM must define liquidity cap
...
- 減碼規則：基本面支持保留，但技術明確偏空，禁止加碼
- 退出規則：工業車用需求、庫存、FCF、12吋廠利用率或併購惡化時下調；站回 275.14 才解除警戒
