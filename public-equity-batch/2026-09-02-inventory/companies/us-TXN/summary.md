...
- 健檢動作：續抱
- 健檢理由：基本面支持保留，但技術破位禁止加碼；20K 不單獨決定賣出
- 近一週觸發：近一週未觸發買進/賣出
- 短線情境：high 289.62；warn 275.14；break 260.66；目前 break
- 失效條件：工業車用需求、庫存、FCF 或 12 吋廠利用率惡化，或持續低於 260.66 時重評
...
- 風險預算：0.0
- 下檔/停損距離：20d_low distance 1.15%; 20D low 250.43 (input only; PM must verify)
- 流動性上限：20D avg trade value 1,526,304,926.65; PM must define liquidity cap
...
- 減碼規則：基本面支持保留，但技術破位禁止加碼；20K 不單獨決定賣出
- 退出規則：工業車用需求、庫存、FCF 或 12 吋廠利用率惡化，或持續低於 260.66 時重評
